...
fun main() {
    val result = slowestKey(intArrayOf(23, 34, 43, 59, 62, 80, 83, 92, 97), "qgkzzihfc")
    println(result.toString())
}

fun slowestKey(releaseTimes: IntArray, keysPressed: String): Char {
    var item = releaseTimes[0]
    var slowestKey = keysPressed[0]

    for (i in 1 until releaseTimes.size) {
        val resultant = releaseTimes[i] - releaseTimes[i - 1]
        if (resultant > item) {
            item = resultant
            slowestKey = keysPressed[i]
        } else if (resultant == item) {
